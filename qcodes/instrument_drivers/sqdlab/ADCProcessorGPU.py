...
class ExtractorGPUForML(Unpacker):
    '''Perform marker extraction on GPU. Does not convert to Float'''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._to_float_gpu = dsp.MarkerExtractionForML(self.parent.gpu_context)
...
class TvModeGPUML(TvMode):
    def __init__(self, name, cl_context=None, cl_queue=None, *args, **kwargs):
        ...
        self.add_submodule('unpacker', ExtractorGPUForML(self, 'ExtractorGPUForML'))
        self.add_submodule('ddc', DigitalDownconversionGPU(self, 'ddc'))
        self.add_submodule('filter', FilterGPU(self, 'filter'))
        self.add_submodule('fft', FFTGPU(self, 'fft'))
        self.add_submodule('sync', Synchronizer(self, 'sync'))
    ...
    def generate(self, source, mean=False, classify=False):
        ...
        * `sum` takes the sum or mean over the iteration axis of the reshaped 
        ...
        for block in source:
            ...
            # truncate & reshape to (iteration, segment, sample, channel)
            repetitions = analog.shape[0] // segments
            analog_trunc = (analog[:repetitions*segments,...]
                            .reshape((repetitions, segments)+analog.shape[1:]))
            yield analog_trunc.get()
    ...
    def __call__(self, source):
        ...
        '''
        # Making a full size array with zeros and inserting data into it as it comes in.
        final_array = np.zeros(self._singleshot_shape, dtype=np.int16)
        iterations_per_block = self._blocksize//self._singleshot_shape[1]
        for i, analog_trunc in enumerate(self.generate(source)):
            final_array[i*iterations_per_block:(i+1)*iterations_per_block] += analog_trunc
        return final_array
