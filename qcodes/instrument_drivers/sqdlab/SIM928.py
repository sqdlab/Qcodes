...
class SIM928_PLX(PrologixGPIBEthernet, Instrument):
    """
    ...
    def __init__(self, name, address, gpib_slot, slot_names=None, **kw):
...
class SIM928_PLX_Proxy(Instrument):
    """
    A driver for Stanford Research Systems SIM 928 DC source modules installed
    in a SIM900 mainframe. Using this module allows you to connect to the mainframe using
    a Pyro4 proxy server.

    Args:
        name (str): An identifier for this instrument, particularly for
            attaching it to a ``Station``.
        remote_proxy (RemoteFPGA): The RemoteFPGA connection of your instrument
        slot_names (Dict[int]): An dictionary that optionally maps slot numbers
            to user-defined module names. Default ``{}``.
        timeout (number): Seconds to allow for responses. Default ``5``.
        metadata (Optional[Dict]): Additional static metadata to add to this
    ...
    def write(self, msg):
        '''
        Wrapper around the write function using the proxy. It writes directly to the mainframe
        
        '''
        return self._proxy.write(None, msg)
        
    def read(self, msg):
        '''
        Wrapper around the read function using the proxy. It writes directly to the mainframe
        
        '''
        return self._proxy.read(None, msg)
        
    def ask(self, msg):
        '''
        Wrapper around the ask function using the proxy. It writes directly to the mainframe
        
        '''
        return self._proxy.ask(None, msg)
        
    def query(self, msg):
        '''
        Wrapper around the query function using the proxy. It writes directly to the mainframe
