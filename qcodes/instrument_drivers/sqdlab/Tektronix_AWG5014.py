...
from qcodes import VisaInstrument, InstrumentChannel, validators as vals
...
class Tektronix_AWG_Channel(InstrumentChannel):
    ...
    def __init__(self, parent, name, channel) -> None:
        ...
        offset_cmd = 'SOURce{}:VOLTage:LEVel:IMMediate:OFFS'.format(self.channel)
        state_cmd = 'OUTPUT{}:STATE'.format(self.channel)
        waveform_cmd = 'SOURce{}:WAVeform'.format(self.channel)
        directoutput_cmd = 'AWGControl:DOUTput{}:STATE'.format(self.channel)
        filter_cmd = 'OUTPut{}:FILTer:FREQuency'.format(self.channel)
        add_input_cmd = 'SOURce{}:COMBine:FEED'.format(self.channel)
        dc_out_cmd = 'AWGControl:DC{}:VOLTage:OFFSet'.format(self.channel)

        # Set channel first to ensure sensible sorting of pars
        self.add_parameter('state'.format(self.channel),
                           label='Status channel {}'.format(self.channel),
                           get_cmd=state_cmd + '?',
                           set_cmd=state_cmd + ' {}',
                           vals=vals.Ints(0, 1),
                           get_parser=int)
        self.add_parameter('amplitude'.format(self.channel),
                           label='Amplitude channel {}'.format(self.channel),
                           unit='Vpp',
                           get_cmd=amp_cmd + '?',
                           set_cmd=amp_cmd + ' {:.6f}',
                           vals=vals.Numbers(0.02, 4.5),
                           get_parser=float)
        self.add_parameter('offset'.format(self.channel),
                           label='Offset channel {}'.format(self.channel),
                           unit='V',
                           get_cmd=offset_cmd + '?',
                           set_cmd=offset_cmd + ' {:.3f}',
                           vals=vals.Numbers(-.1, .1),
                           get_parser=float)
        self.add_parameter('waveform'.format(self.channel),
                           label='Waveform channel {}'.format(self.channel),
                           get_cmd=waveform_cmd + '?',
                           set_cmd=waveform_cmd + ' "{}"',
                           vals=vals.Strings(),
                           get_parser=parsestr)
        self.add_parameter('direct_output'.format(self.channel),
                           label='Direct output channel {}'.format(self.channel),
                           get_cmd=directoutput_cmd + '?',
                           set_cmd=directoutput_cmd + ' {}',
                           vals=vals.Ints(0, 1))
        self.add_parameter('add_input'.format(self.channel),
        ...
        self.add_parameter('DC_out'.format(self.channel),
                           label='DC output level channel {}'.format(self.channel),
                           unit='V',
                           get_cmd=dc_out_cmd + '?',
                           set_cmd=dc_out_cmd + ' {}',
...
class Tektronix_AWG_Marker(InstrumentChannel):
        def __init__(self, parent, name, channel) -> None:
            ...
            m_high_cmd = ('SOURce{}:MARKer{}:VOLTage:' +
                          'LEVel:IMMediate:HIGH').format(self.parent.channel, self.channel)
...
class Tektronix_AWG5014(VisaInstrument):
    ...
    def __init__(self, name, address, timeout=180, num_channels=4, **kwargs):
        ...
        self.add_submodule('ch1', Tektronix_AWG_Channel(self, 'ch1', 1))
        self.add_submodule('ch2', Tektronix_AWG_Channel(self, 'ch2', 2))
        self.add_submodule('ch3', Tektronix_AWG_Channel(self, 'ch3', 3))
        self.add_submodule('ch4', Tektronix_AWG_Channel(self, 'ch4', 4))
    ...
    def load_sequence(self, path, filename):
        '''
        Use this command to directly load a sequence file stored in a directory on the 
        host computer that is mapped on the AWG. Uses mtab to determine how to access 
        the files on the host.
        
        Input:
            fn (str) - full file name and path of a sequence file
        '''
        #host_path, host_file = os.path.split(fn)
        host_path = path
        host_file = filename
        #host_fullpath = os.path.join(host_path, host_file)
        host_fullpath = os.path.realpath(host_path)
        for host_map, awg_map in self._mtab.items():
            if not host_map.endswith(os.path.sep):
                host_map += os.path.sep
            if host_fullpath.startswith(host_map):
                awg_fullpath = host_fullpath.replace(host_map, awg_map, 1)
                awg_drive, awg_path = os.path.splitdrive(awg_fullpath)
                self.load_remote_sequence(host_file, drive=awg_drive, path=awg_path)
                return
        raise ValueError('Local path {0} is not accessible from the AWG.'.format(host_fullpath))

    def load_remote_sequence(self, filename, drive='Z:', path='\\'):
        '''
        Use this command to directly load a sequence file.
        ...
        Output:
            None
        '''
        logging.debug(__name__ + ' : Load sequence file %s%s%s' % (drive, path, filename))
        self.write('SOUR:FUNC:USER "%s/%s","%s"' % (path, filename, drive))
        # TODO: should detect errors

    def set_mtab(self, value):
        '''
        Set host directory to AWG local directory mapping table.
        
        Input:
            {host directory: awg directory}
        '''
        self._mtab = value
        
    def get_mtab(self):
        ''' Get host directory to AWG local directory mapping table. '''
        return self._mtab